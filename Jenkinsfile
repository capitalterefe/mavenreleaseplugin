    ...
    stages {
        ...
        stage('Build') {
            steps {
             ...
             def versions = sh(script: "curl -s https://repo1.maven.org/maven2/org/brutusin/wava/maven-metadata.xml | grep -Po '(?<=<version>)([0-9\\.]+(-SNAPSHOT)?)' | sort --version-sort -r | head -n 5", returnStdout: true).trim().split("\r?\n")
                     //   def versions = sh(script: "curl -s https://repo1.maven.org/maven2/org/brutusin/wava/maven-metadata.xml", returnStdout: true).trim()
...
//                   def versions = sh(script: "curl -s ${metad} | grep '<version>.*</version>' | sed -e 's#\(.*\)\(<latest>\)\(.*\)\(</latest>\)\(.*\)#\3#g'", returnStdout: true)
    			    ...
    			    IP_HUB = "${versions[top-2]}"
    			    NETWORK = "${versions[top-1]}"
                    println("Grid parameters: $IP_HUB / $NETWORK")

                    println("Grid yuy: $top")
